...
public class lootTableLoadEvent {
	...
	public void onLootTableLoadEvent(LootTableLoadEvent event) {
		if (event.getName().equals(LootTableList.ENTITIES_BAT)) {
			...
			if (pool == null) {
				pool = new LootPool(new LootEntry[0], noCondition, new RandomValueRange(0, 2), new RandomValueRange(0),
						"main");
				event.getTable().addPool(pool);
			}

			LootFunction amount = new SetCount(noCondition, new RandomValueRange(1, 2));
			pool.addEntry(new LootEntryItem(ModItems.claw, 1, 0, new LootFunction[] { amount }, noCondition,
					References.modId + ":claws"));
		}
	}
}
