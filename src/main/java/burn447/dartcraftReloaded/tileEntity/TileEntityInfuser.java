...
public class TileEntityInfuser extends TileEntity
		implements ITickable, ICapabilityProvider, ITileEntityProvider, IFluidHandler {
...
//            return addGrindingModifier(stack);
		...
		if (stack.getItem() instanceof ItemForceRod) {
			if (!stack.getCapability(CAPABILITY_FORCEROD, null).isRodOfHealing(1)) {
				...
				return true;
					...
					&& stack.getCapability(CAPABILITY_FORCEROD, null).isRodOfHealing(1)) {
				stack.getCapability(CAPABILITY_FORCEROD, null).setRodOfHealing(true, 2);
				return true;
			} else if (!stack.getCapability(CAPABILITY_FORCEROD, null).isRodOfHealing(3)
					&& stack.getCapability(CAPABILITY_FORCEROD, null).isRodOfHealing(2)
					&& stack.getCapability(CAPABILITY_FORCEROD, null).isRodOfHealing(1)) {
	...
	private boolean addSturdyModifier(ItemStack stack) {
		Item st = stack.getItem();
		if (st instanceof ItemForceSword || st instanceof ItemForceAxe || st instanceof ItemForceShovel
				|| st instanceof ItemForcePickaxe || st instanceof ItemForceRod) {
	...
	private boolean addSpeedModifier(ItemStack stack) {
		if (stack.getItem() instanceof ItemForceShovel || stack.getItem() instanceof ItemForcePickaxe
